...
import { resolveAudioSource } from "./audioResources";
...
export function useChartAudio(options: UseChartAudioOptions): ChartAudioApi {
  ...
  const decodeSound = useCallback(
    async (soundName: string): Promise<AudioBuffer | null> => {
      const game = trackRef.current?.game ?? "adofai";
      const cacheKey = `${game}:${soundName}`;
      const existing = soundBuffersRef.current.get(cacheKey);
      if (existing) {
        return existing;
      }

      const source = await resolveAudioSource(game, soundName);
      const candidates = source ? [source] : [];

      for (const url of candidates) {
        try {
          return await decodeUrl(url, soundBuffersRef.current, cacheKey);
        } catch {
          continue;
        }
      }
  ...
  const load = useCallback(
    async (track: TrackSummary, timeline: AudioTimeline) => {
      const source =
        track.audioPath?.startsWith("rd:") || track.audioPath?.startsWith("adofai:")
          ? null
          : toAssetUrl(track.audioPath);
      const resolvedSource = source ?? (await resolveAudioSource(track.game, track.audioPath ?? ""));
      if (!resolvedSource) {
      ...
      const buffer = await decodeUrl(resolvedSource, mainBuffersRef.current, track.audioPath ?? resolvedSource);
