...
pub fn parse_level_text(raw: &str, lenient: bool) -> Result<ParsedLevel, String> {
    ...
    match serde_json::from_str::<Value>(text) {
        ...
        Err(strict_err) if lenient => json5::from_str::<Value>(&repair_lenient_json_text(text))
            ...
            .map_err(|_| "谱面文件格式异常，暂时无法读取".to_string()),
        Err(_) => Err("谱面文件格式异常，暂时无法读取".to_string()),
    }
}
...
pub fn number_setting(settings: &BTreeMap<String, Value>, key: &str, fallback: f64) -> f64 {
    value_as_f64(settings.get(key), fallback)
}
...
pub fn event_bar(event: &Value) -> i32 {
    value_as_f64(event.get("bar"), 1.0).round().max(1.0) as i32
}

pub fn event_beat(event: &Value) -> f64 {
    value_as_f64(event.get("beat"), 1.0).max(1.0)
}

pub fn event_sort_order(event: &Value) -> i32 {
    value_as_f64(event.get("sortOrderOffset"), 0.0).round() as i32
}
...
pub fn value_as_i64(value: Option<&Value>, fallback: i64) -> i64 {
    value_as_f64(value, fallback as f64).round() as i64
}

pub fn value_as_bool(value: Option<&Value>, fallback: bool) -> bool {
    value
        .and_then(|value| match value {
            Value::Bool(value) => Some(*value),
            Value::String(text) => text.parse::<bool>().ok(),
...
pub fn sound_ref_at(event: &Value, key: &str, fallback: &str) -> SoundRef {
    decode_sound_ref(event.get(key), fallback)
}

pub fn decode_sound_ref(value: Option<&Value>, fallback: &str) -> SoundRef {
    match value {
        Some(Value::Object(map)) => {
            let filename = value_as_string(map.get("filename"), fallback);
            SoundRef {
                filename,
                volume: (value_as_f64(map.get("volume"), 100.0) / 100.0) as f32,
                pitch: (value_as_f64(map.get("pitch"), 100.0) / 100.0) as f32,
                offset_ms: value_as_f64(map.get("offset"), 0.0),
                used: value_as_bool(map.get("used"), true),
            }
        }
        Some(Value::String(filename)) => SoundRef::new(filename),
        _ => SoundRef::new(fallback),
    }
}
